...
namespace Explorevia.Repository
{
    public class AuthRepository : IAuthRepository
    {
        ...
        public async Task<bool> SendHotelRegisterRequest(HotelOwnerRegisterViewModel hotel)
        {
            ...
            // Validate Hotel License using ML model
            byte[] ownerNationalID;
            using (var ms = new MemoryStream())
            {
                await hotel.OwnerIdCard.CopyToAsync(ms);
                ownerNationalID = ms.ToArray();
            }

            var input = new ExploreviaML.ModelInput()
            {
                ImageSource = ownerNationalID
            };
            var prediction = ExploreviaML.Predict(input);
            if (prediction.PredictedLabel != "IDCard")
            {
                return false;
            }
        ...
        public async Task<string> Login(LoginViewModel loginvm)
        {
            ...
            // Check password
            bool isPasswordCorrect = await _userManager.CheckPasswordAsync(user, loginvm.Password);
